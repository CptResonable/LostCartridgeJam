...
public class WallrunController : MonoBehaviour {
    ...
    public void AttemptWallRun() {
        ...
        if (!isWallRunning && !wallClimbOnCooldown) {
            Vector3 cameraVelocityMiddleVector = Vector3.Slerp(Vector3.ProjectOnPlane(character.head.tCameraBase.forward, Vector3.up), smoothCharacterHorizontalVelocity, 0.5f);
    ...
    private IEnumerator HorizontalRunCorutine(float duration) {
        ...
        while (t < 1) {
            ...
            wallCameraAngle = Vector3.SignedAngle(Vector3.ProjectOnPlane(character.head.tCameraBase.forward, Vector3.up), Vector3.ProjectOnPlane(-wallHit.normal, Vector3.up), Vector3.up);

            Vector3 camForwardProj = Vector3.ProjectOnPlane(character.head.tCameraBase.forward, character.locomotion.wallrunController.wallUpVector).normalized;
